  ...
  _createClass(Component, [{
    ...
    value: function getModifiedSchema(schema, defaultSchema, recursion) {
      ...
      _lodash.default.each(schema, function (val, key) {
        ...
        } else if (!recursion && key === 'type' || !recursion && key === 'key' || !recursion && key === 'label' || !recursion && key === 'input' || !recursion && key === 'tableView' || !recursion && key === 'cSchema' || val !== '' && !defaultSchema.hasOwnProperty(key) || val !== '' && val !== defaultSchema[key]) {
          modified[key] = val;
        }
      });
